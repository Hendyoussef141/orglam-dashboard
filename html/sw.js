const CACHE_NAME = 'orglam-shell-v11';
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    ...
    vibrate: [200, 100, 200],
    data: { url: data.url || './Orglam Dashboard.html' },
  };
...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    caches.match(event.request).then((cached) => {
      ...
      // Whichever answers first wins, with the cache guaranteed to answer by 2.5s.
      return Promise.race([
